...
function get_sampler(instrument) {
    if (TONE_SAMPLERS[instrument] === undefined) {
        TONE_SAMPLERS[instrument] = new Tone.Sampler({
            urls: INSTRUMENT_INFO[instrument].sample_files,
            release: 1,
            baseUrl: INSTRUMENT_INFO[instrument].base_url,
        }).toDestination();
...
function use_legacy(color) {
    if (INSTRUMENT_INFO[STATE.current_instrument].legacy) {
        if (color === undefined || Object.keys(CHORDS_TONE).findIndex(color) < FIRST_BLACK_INDEX) {
            return true;
        }
    }
    return false;
